...
const DataProvider = ({ children }) => {
    ...
    const value = { users, products, customers, orders, payments, deleteDocument, addProduct, addCustomer, addBulkOrders, updateOrder, updateMultipleOrderFields, addPayment };
...
const AddOrderPage = () => {
    ...
    const initialFormState = { OrderDate: getTomorrow().toISOString().split('T')[0], CustomerName: '', ProductSKU: '', Quantity: '', UnitPrice: '', Comment: '', isBlack: false };
    const [formState, setFormState] = useState(initialFormState); const [tempOrderList, setTempOrderList] = useState([]); const [showCustomerSuggestions, setShowCustomerSuggestions] = useState(false);
    ...
    const handleLoadLastOrder = () => { if (!formState.CustomerName) return; const lastOrder = orders.filter(o => o.CustomerName === formState.CustomerName).sort((a,b) => b.OrderDate - a.OrderDate)[0]; if (lastOrder) { setFormState(prev => ({ ...prev, ProductSKU: lastOrder.ProductSKU, Quantity: lastOrder.Quantity, Comment: lastOrder.Comment, isBlack: lastOrder.isBlack })); } else { alert(t.noOrdersFound); } };
    ...
    return (<div className="space-y-6"><div className="bg-white p-6 rounded-lg shadow-md border"><h2 className="text-xl font-bold mb-4">{t.addNewOrder}</h2><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-start">
        ...
        <div><label className="text-xs">{t.product}</label><select name="ProductSKU" value={formState.ProductSKU} onChange={handleChange} className="w-full p-2 border rounded-md bg-white"><option value="">{t.selectProduct}</option>{products.map(p => <option key={p.id} value={p.ProductSKU}>{p.ProductName}</option>)}</select></div>
