 ...
 * - ✅ Payment inclusion window starts 2025-04-29 (per requirement). Waybills still use 2025-04-29.
 */
...
// Payments must include 2025-04-29 and 2025-04-29
...
const CustomerAnalysisPage = () => {
  ...
  const [rememberedWaybills, setRememberedWaybills] = useState(() => {
    ...
    Object.entries(stored).forEach(([id, wb]) => {
      if (wb.isAfterCutoff && wb.date && new Date(wb.date) > new Date('2025-04-29')) {
        cleaned[id] = wb;
      }
    });
    return cleaned;
  ...
  const parseExcelDate = useCallback((dateValue) => {
    ...
    if (typeof dateValue === 'number') {
      ...
      // Excel serial 45776 = 2025-04-29, so we need > 45776 for 2025-04-29+
